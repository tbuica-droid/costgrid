...
provider and everything keeps working. That is deliberate — a tool that is
hard to remove is a tool you have to be talked into.
...
2. Put your provider key in `.env` (the quickstart shows a way that keeps it out
   of your shell history).
3. `npx tsx packages/cli/src/main.ts init` — creates the local tenant, prints an
   API key once.
4. `npm run gateway` — listens on `127.0.0.1:8787`.
5. Point one service at it and make a call. Open the dashboard.
...
`x-costgrid-department`. Do not set a single enforcement rule. The question this
week is only: *does the dashboard tell you something the provider console does
not?* If it does not, stop here and keep your money.
...
whoever owns the budget. That conversation is the actual product. If it does not
happen, or lands flat, CostGrid is not worth a line item for you.
...
- **It is in the request path.** If the gateway is down, calls through it fail.
  That is the cost of being able to refuse a call rather than report on it
  afterwards. Run it beside the service that calls it.
- **One node, SQLite.** Fine for a team; not yet a high-availability deployment.
  The schema is Postgres-portable but the swap has not been made.
- **No payment processor.** Plan changes record intent; there is nothing to
  charge your card because nothing charges cards yet.
...
of bug there is, and worth a report even if you are only evaluating. A cost tool
that is quietly wrong is worse than no cost tool.
